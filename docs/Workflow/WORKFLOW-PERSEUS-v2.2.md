...
Daily Perseus development happens inside per-branch Git worktrees, each bound to its own PostgreSQL 18 database cloned from a golden template in ~200 ms. The orchestrator is **`gtr` (CodeRabbit)**, configured via committed `.gtrconfig`. When a developer or Claude Code session runs `git gtr new <branch>`, the **declarative `postCreate` hook** fires (`postCreate.sh` → `provision-branch-db.sh`) creating the per-branch DB and writing a `.env` file. Developers or Claude Code then enter a TMUX session (`gtr-tmux <branch>`) with 4 panes: Claude Code agent, editor, psql, and a logs/test runner. pgTAP tests run via `run-pgtap.sh` which automatically uses **ephemeral databases** (cycle < 3 s, validated) for procedures with internal `COMMIT`/`ROLLBACK` and `BEGIN/ROLLBACK` for stateless functions. Worktree teardown via `git gtr rm <branch>` triggers the **declarative `preRemove` hook** (`preRemove.sh` → `deprovision-branch-db.sh`) which dumps the DB as a safety net before dropping it.
...
| `PERSEUS_BASE` | `$REPO_BASE_DIR/github.com/pierreribeiro/olympios` | Convenience env var for this specific repo |

Add to `~/.zshrc`:

```bash
export REPO_BASE_DIR="$HOME/dev/repos"
export PERSEUS_BASE="$REPO_BASE_DIR/github.com/pierreribeiro/olympios"
...
#  Worktree:  ~/dev/repos/github.com/pierreribeiro/olympios/feat-users-rbac
# =============================================================================
...
DATABASE_URL=postgres://perseus@localhost:5432/perseus_feat_users_rbac
...
localhost:5432:*:perseus_owner:<your-password-here>
```
...
REPO_URL="${1:-${PERSEUS_REPO_URL:-git@github.com:pierreribeiro/olympios.git}}"
...
# 5. Set up .pgpass
echo "localhost:5432:*:perseus:<your-password>" >> ~/.pgpass
chmod 600 ~/.pgpass
...
cd $PERSEUS_BASE/main
git config --local perseus.pg.user perseus
git config --local perseus.pg.template dev_template
...
pg_restore -v -d "postgres://perseus@localhost/dev_template" ~/dumps/perseus_staging.dump
  ...
  pgtap:
    ...
    steps:
      ...
      - run: |
          ...
          psql -h localhost -U perseus -c "CREATE DATABASE dev_template;"
          psql -h localhost -U perseus -d dev_template -f schema/full-schema.sql
          psql -h localhost -U perseus -c "UPDATE pg_database SET datistemplate=true WHERE datname='dev_template';"
          psql -h localhost -U perseus -c "CREATE DATABASE perseus_ci WITH TEMPLATE dev_template;"
          ./scripts/run-pgtap.sh
        env:
